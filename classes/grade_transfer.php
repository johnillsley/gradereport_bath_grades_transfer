...
class grade_report_transfer_grade_transfer
{
    ...
    public function prepare_grade_transfer($mappingid, $userids = array()) {
        ...
        if (1 == 1) { // TODO - replace with mapping is valid check - see above
            ...
            if (count($userids) > 0) {
                ...
                $checkedgrades = $this->pre_transfer_checks($grades_structure, $userids);

                //$transfer_outomes = $assessment_data->export_grades( $checkedgrades ); // uses moodle.user.id in key for transfer_outcome return
                // TODO - REMOVE THIS FAKE TRANSFER SUCCESS
                foreach ($checkedgrades as $k => $v) {
                    $transferoutcomes[$k] = $v;
                    $transferoutcomes[$k]->status = TRANSFER_SUCCESS;
                }
                // END OF FAKE TRANSFER SUCCESS

                if (count($transferoutcomes) > 0) {
                    $this->log_outcomes($transferoutcomes);
                }
            }
        } else {
            // Mapping not valid
    ...
    private function pre_transfer_checks($grades_structure, $userids = array()) {
        ...
        foreach ($userids as $userid) {
            ...
            $studentspr = $this->get_external_reference($userid);
            $moodlegrade = $this->get_moodle_grade($userid);
            $outcomeid = 0;

            if (empty($moodlegrade->finalgrade)) {
                $outcomeid = TRANSFER_NO_GRADE; // No grade to transfer

            } elseif ($moodlegrade->rawgrademax != MAX_GRADE) {
                $outcomeid = GRADE_NOT_OUT_OF_100; // Assessment not graded out of 100

            } elseif (empty($grades_structure[$studentspr])) {
                $outcomeid = NOT_IN_SITS_STRUCTURE; // Grade not expected by SITS - not in structure

            } elseif (!empty($grades_structure[$studentspr]->mark)) {
                $outcomeid = GRADE_ALREADY_EXISTS; // Grade already exists in SITS

            } else {
                // Export OK to go
                $grades_structure[$studentspr]->mark = $moodlegrade->finalgrade; // add grade ready for export
                $checkedgrades[$userid] = $grades_structure[$studentspr]; // Use moodle.user.id for export
            }

            if ($outcomeid != 0) {
                // Log why transfer is not being attempted
                $this->outcomes[$userid] = $outcomeid;
                $this->transfer_log->add($this->mapping, $userid, $outcomeid, $moodlegrade->finalgrade);
            }
        }
        return $checkedgrades;
    }

    private function log_outcomes($transferoutcomes) {
        global $DB;
        ...
        foreach ($transferoutcomes as $k => $transfer_outcome) {
            $userid = $k;
